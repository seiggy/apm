...
public static class PrimitiveParser
{
    ...
    /// Returns typed metadata and body content.
    /// </summary>
    internal static (PrimitiveFrontmatter Metadata, string Content) ParseFrontmatter(string text)
        => ParseFrontmatter<PrimitiveFrontmatter>(text);

    /// <summary>
    /// Parse YAML frontmatter from markdown content into a specific type.
    /// Use this when you need access to fields beyond the base PrimitiveFrontmatter.
    /// </summary>
    internal static (T Metadata, string Content) ParseFrontmatter<T>(string text) where T : class, new()
    {
        ...
        try
        {
            metadata = YamlFactory.CamelCaseDeserializer.Deserialize<T>(yamlBlock)
                       ?? new T();
        }
        catch
        {
    ...
    private static Chatmode ParseChatmode(string name, string filePath, PrimitiveFrontmatter metadata, string content, string? source)
    {
        return new Chatmode
        {
            Name = name,
    ...
    private static Instruction ParseInstruction(string name, string filePath, PrimitiveFrontmatter metadata, string content, string? source)
    {
    ...
    private static Context ParseContext(string name, string filePath, PrimitiveFrontmatter metadata, string content, string? source)
    {
        return new Context
        {
            Name = name,
