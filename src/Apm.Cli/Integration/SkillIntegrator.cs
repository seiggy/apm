...
public class SkillIntegrator
{
    ...
    internal Dictionary<string, string> ParseSkillMetadata(string filePath)
    {
        try
        {
            var (metadata, _) = PrimitiveParser.ParseFrontmatter<SkillFileFrontmatter>(File.ReadAllText(filePath));
            if (metadata.Metadata is { } meta)
            {
                return new Dictionary<string, string>
                {
                    ["Version"] = meta.ApmVersion ?? "",
                    ["Commit"] = meta.ApmCommit ?? "",
                    ["Package"] = meta.ApmPackage ?? "",
                    ["ContentHash"] = meta.ApmContentHash ?? ""
                };
            }
            return [];
        }
